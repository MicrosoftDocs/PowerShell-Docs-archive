...
ms.date: 02/20/2019
online version: https://learn.microsoft.com/previous-versions/powershell/module/cimcmdlets/register-cimindicationevent?view=powershell-5.0&WT.mc_id=ps-gethelp
schema: 2.0.0
...
subscription using the `Get-Event` cmdlet. For more information, see [Get-Event](../Microsoft.PowerShell.Utility/Get-Event.md).
...
This set of commands uses a query to subscribe to an event generated whenever there is a change in
...
holds the script block for **Action**, which uses the `$event` variable to access the event received
from CIM.
...
The commands specified by this parameter run when an event is raised, instead of sending the event
